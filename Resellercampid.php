 ...
 */
class Registrar_Adapter_Resellercampid extends Registrar_AdapterAbstract
{
    /**
     * config
     * @return boolean
    ...
    public function __construct($options)
    {
        ...
        if(isset($options['userid']) && !empty($options['userid'])) {
            ...
            throw new Registrar_Exception('Domain registrar "Resellercampid" is not configured properly. Please update configuration parameter "Resellercampid Reseller ID" at "Configuration -> Domain registration".');
        }
    ...
    {
        return array(
            'label'     =>  'Manages domains on Resellercampid via API. Resellercampid requires your server IP in order to work. Login to the Resellercampid control panel (the url will be in the email you received when you signed up with them) and then go to Settings > API and enter the IP address of the server where BoxBilling is installed to authorize it for API access.',
            'form'  => array(
                'userid' => array('text', array(
                            'label' => 'Reseller ID. You can get this at Resellercampid control panel Settings > Personal information > Primary profile > Reseller ID',
                            'description'=> 'Resellercampid Reseller ID'
                        ),
                     ),
                'api-key' => array('password', array(
                            'label' => 'Resellercampid API Key',
                            'description'=> 'You can get this at Resellercampid control panel, go to Settings -> API',
                            'required' => false,
                        ),
                     ),
            ),
    ...
    protected function _makeRequest($url, $params = array(), $method = 'get')
    {
        ...
        # langsung cek respon
        if (!$response) {
            throw new Registrar_Exception($curl_error ? $curl_error : "Unable to request data from resellercamp server (".$request_url.")");
        }
        ...
        if (strpos($header, "404 Not Found")) {
            throw new Registrar_Exception("Unable to request data from resellercamp server, URL is not valid (".$request_url.")");
        }
